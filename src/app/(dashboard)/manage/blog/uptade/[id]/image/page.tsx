...
import { BlogItem, FileType, UploadedFileMeta } from "@/src/services/interface";
...
import { blog_list } from "@/src/services/interface/constant";
import {
  getBlogById,
  updateBlogImage,
} from "@/src/actions/client/blog.actions";
...
export default function UpdateImagePage() {
  ...
  const { data: existingData, refetch } = useServerQueryById<BlogItem>(
    blog_list,
    getDataWrapper,
    id,
    { locale: "az" }
  ...
  const { mutate: deleteFile } = useDeleteData<FileType>({
    invalidateKeys: [blog_list, `${blog_list}/${id}`],
  });
  ...
  const onSubmit = async (data: ImgInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await updateBlogImage(existingData.documentId, {
