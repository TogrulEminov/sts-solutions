...
import {
  getExpertiseById,
  updateExpertise,
} from "@/src/actions/client/expertise.actions";
import { expertise_list } from "@/src/services/interface/constant";
...
export default function UptadeContent() {
  ...
  const getDataWrapper = async () => {
    const result = await getExpertiseById({
  ...
  const generalForm = useForm<UpdateExpertiseInput>({
    resolver: zodResolver(uptadeExpertiseSchema),
    values: formValues,
  });
  const {
    handleSubmit,
  ...
  const onSubmit = async (data: UpdateExpertiseInput) => {
    startTransition(async () => {
      const result = await updateExpertise(id as string, data);
