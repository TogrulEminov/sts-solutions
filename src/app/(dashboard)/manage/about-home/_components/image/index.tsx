...
import { updateHomeAboutImage } from "@/src/actions/client/about-home.actions";
...
export default function AboutUpdateImagePage({ existingData, refetch }: Props) {
  ...
  const { mutate: deleteFile } = useDeleteData<FileType>({
    invalidateKeys: [about_home_list, `${about_home_list}/${id}`],
  });
  ...
  const onSubmit = async (data: ImgInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await updateHomeAboutImage(existingData.documentId, {
