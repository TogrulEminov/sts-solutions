...
export default function AdvantagesUpdateImagePage() {
  ...
  const { data: existingData, refetch } = useServerQueryById<ConnectionItem>(
    partners_main_list,
    getCategoryWrapper,
    id,
    { locale: "az" }
  ...
  const { mutate: deleteFile } = useDeleteData<FileType>({
    invalidateKeys: ["partners", `partners/${id}`],
  });
  ...
  const onSubmit = async (data: ImgInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await updatePartnersImage(existingData.documentId, {
