...
import { Employee, FileType, UploadedFileMeta } from "@/src/services/interface";
...
export default function UpdateImagePage() {
  ...
  const { data: existingData, refetch } = useServerQueryById<Employee>(
    employee_list,
    getDataWrapper,
    id,
    { locale: "az" }
  ...
  const { mutate: deleteFile } = useDeleteData<FileType>({
    invalidateKeys: [employee_list, `${employee_list}/${id}`],
  });
  ...
  const onSubmit = async (data: ImgInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await uptadeEmployeeImage(existingData.documentId, {
