...
import {  FileType, ServiceItem, UploadedFileMeta } from "@/src/services/interface";
...
import { getServicesById, uptadeServicesImage } from "@/src/actions/client/services.actions";
...
export default function UpdateImagePage() {
  ...
  const { data: existingData, refetch } = useServerQueryById<ServiceItem>(
    service_list,
    getDataWrapper,
    id,
    { locale: "az" }
  ...
  const { mutate: deleteFile } = useDeleteData<FileType>({
    invalidateKeys: [service_list, `${service_list}/${id}`],
  });
  ...
  const onSubmit = async (data: ImgInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await uptadeServicesImage(existingData.documentId, {
