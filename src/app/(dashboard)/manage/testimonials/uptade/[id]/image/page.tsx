...
export default function TestimonialsUpdateImagePage() {
  ...
  const { data: existingData, refetch } = useServerQueryById<TestimonialsItem>(
  ...
  const { mutate: deleteFile } = useDeleteData<FileType>({
    invalidateKeys: [testimonials_main_list, `${testimonials_main_list}/${id}`],
  });
  ...
  const onSubmit = async (data: ImgInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await updateTestimonialsImage(existingData.documentId, {
