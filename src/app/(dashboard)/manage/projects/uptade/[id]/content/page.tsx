...
import { CustomLocales, Projects } from "@/src/services/interface";
...
import {
  getProjectsById,
  uptadeProjects,
} from "@/src/actions/client/projects.actions";
...
export default function UpdateContent() {
  ...
  const getDataWrapper = async () => {
    ...
    return {
      data: result.data as Projects | undefined,
      message: result.message,
      code: result.code,
    };
  };
  const { data: existingData } = useServerQueryById<Projects>(
    projects_list,
    getDataWrapper,
    id,
    { locale }
  ...
  const generalForm = useForm<UpdateProjectsInput>({
    mode: "onChange",
    resolver: zodResolver(uptadeProjectsSchema),
    values: formValues,
  });
  const {
    handleSubmit,
  ...
  const onSubmit = async (data: UpdateProjectsInput) => {
    startTransition(async () => {
      const result = await uptadeProjects(id as string, data);
