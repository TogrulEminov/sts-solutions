...
import { FileType, Projects, UploadedFileMeta } from "@/src/services/interface";
...
export default function GalleryUpdateImagePage() {
  ...
  const { data: existingData, refetch } = useServerQueryById<Projects>(
    projects_list,
    getCategoryWrapper,
    id,
    { locale: "az" }
  ...
  const { mutate: deleteFile } = useDeleteData<FileType>({
    invalidateKeys: [projects_list, `${projects_list}/${id}`],
  });
  ...
  const onSubmit = async (data: GalleryInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await uptadeProjectsImages(existingData.documentId, {
