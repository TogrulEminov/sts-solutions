...
import { FileType, IAbout, UploadedFileMeta } from "@/src/services/interface";
...
import { updateAboutMainImage } from "@/src/actions/client/about-main.actions";

interface Props {
  existingData: IAbout | undefined;
  refetch: () => void;
}
export default function AboutUpdateImagePage({ existingData, refetch }: Props) {
  ...
  const generalForm = useForm<ImgInput>({
  ...
  const { mutate: deleteFile } = useDeleteData<FileType>({
    invalidateKeys: [about_main_list, `${about_main_list}/${id}`],
  });
  ...
  const onSubmit = async (data: ImgInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await updateAboutMainImage(existingData.documentId, {
  ...
  const handleSetUploadedFile = (
    ...
    generalForm.setValue("imageId", fileId, {
      shouldValidate: true,
      shouldDirty: true,
    });
  };
