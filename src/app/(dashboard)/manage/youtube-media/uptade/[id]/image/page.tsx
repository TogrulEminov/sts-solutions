...
type UpdateYoutubeImageInput = z.infer<typeof imgSchema>;
...
export default function YoutubesUpdateImagePage() {
  ...
  const generalForm = useForm<UpdateYoutubeImageInput>({
  ...
  const { mutate: deleteFile } = useDeleteData<FileType>({
    invalidateKeys: [youtube_main_list, `${youtube_main_list}/${id}`],
  });

  const onSubmit = async (data: UpdateYoutubeImageInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await updateYoutubeImage(existingData.documentId, {
