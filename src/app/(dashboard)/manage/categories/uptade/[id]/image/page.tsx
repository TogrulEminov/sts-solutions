...
type UpdateCategoryImageInput = z.infer<typeof imgSchema>;
...
export default function CategoriesUpdateImagePage() {
  ...
  const { data: existingData, refetch } = useServerQueryById<CategoryItem>(
    `categories`,
    getCategoryWrapper,
    id,
    { locale: "az" }
  ...
  } = useForm<UpdateCategoryImageInput>({
  ...
  const { mutate: deleteFile } = useDeleteData<FileType>({
    invalidateKeys: ["categories", `categories/${id}`],
  });

  const onSubmit = async (data: UpdateCategoryImageInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await uptadeCategoryImage(existingData.documentId, {
