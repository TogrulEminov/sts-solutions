...
export async function getServicesSubCategory({
  page,
  pageSize,
  query,
  locale,
  ...
  const whereClause: Prisma.ServicesSubCategoryWhereInput = {
    isDeleted: false,
    translations: {
      some: {
        locale: locale,
  ...
  const [data, totalCount] = await Promise.all([
    db.servicesSubCategory.findMany({
      where: whereClause,
      select: {
        id: true,
        status: true,
    ...
    db.servicesSubCategory.count({ where: whereClause }),
  ]);
...
export async function getServicesSubCategoryById({ locale, id }: GetByIDProps) {
  try {
    const byUuid = isUuid(id);
    const whereClause: Prisma.ServicesSubCategoryWhereInput = byUuid
      ? {
          isDeleted: false,
          documentId: id,
        }
...
export async function createServicesSubCategory(
  ...
  try {
    const validateData = createServiceSubCategorySchema.safeParse(input);
    ...
    const existingData = await db.servicesSubCategory.findFirst({
      where: {
        isDeleted: false,
        translations: {
    ...
    const newData = await db.servicesSubCategory.create({
      data: {
        gallery: {
...
export async function uptadeServicesSubCategory(
  id: string,
  ...
  try {
    ...
    const existingCategory = await db.servicesSubCategory.findUnique({
      where: {
        documentId: id,
        isDeleted: false,
      },
    ...
    const parsedInput = uptadeServiceSubCategorySchema.safeParse(input);
    ...
    const uptadeData = await db.$transaction(
      async (prisma) => {
        const updatedData = await prisma.servicesSubCategory.update({
          where: { documentId: id },
          data: {
            translations: {
              upsert: {
...
export async function uptadeServicesSubCategoryImage(
  id: string,
  input: ImgInput
): Promise<ActionResult> {
  const { user, error } = await checkAuthServerAction([
  ...
  try {
    const existingData = await db.servicesSubCategory.findUnique({
    ...
    const uptadeData = await db.servicesSubCategory.update({
      where: { documentId: id },
      data: {
...
export async function uptadeServicesSubCategoryImages(
  id: string,
  input: GalleryInput
): Promise<ActionResult> {
  const { user, error } = await checkAuthServerAction([
